...
FEATURE_DEFS = {
    ...
    # ── Sub-bar trade sequencing (new collector only, 2026-06-21+) ─────────
    # These require data collected with the updated binance_live_orderbook.py.
    # Silently dropped as NaN if old CSV files are mixed in.
    'trade_side_open':      'First trade in bar: +1=buy, -1=sell',
    'trade_side_close':     'Last trade in bar: +1=buy, -1=sell',
    'trade_side_momentum':  'last_side - first_side: +2=sell→buy, -2=buy→sell, 0=no change',
    'largest_trade_side':   'Side of largest single trade: +1=buy, -1=sell',
    'largest_trade_rel':    'Largest trade qty / total bar volume (whale concentration)',
    'buy_accel':            'Buy volume acceleration: late half minus early half (positive=accelerating)',
    'sell_accel':           'Sell volume acceleration: late half minus early half (positive=accelerating)',
    'flow_accel':           'Net flow acceleration: (buy_late-sell_late) - (buy_early-sell_early)',
    'buy_count_accel':      'Buy trade count acceleration: late minus early halves',
    'late_imbalance':       'Flow imbalance in late half of bar only (most recent signal)',
}

# Original 25 features (pre sub-bar collector update, 2026-06-21).
# Used as baseline in --compare mode to measure sub-bar feature value.
ORIGINAL_FEATURES = [
...
# Exact set of raw CSV columns needed by prepare() to compute all features.
# All other columns (~370 out of ~400) are dropped at load time to save memory.
# Pattern columns (liq depths, optional) are resolved dynamically at load time.
COLS_NEEDED_BASE = {
    # Timestamps and datetimes
    'spot_datetime', 'future_datetime',
    'spot_timestamp', 'future_timestamp',

    # Price close — used for target and close_price
    'future_bid_close', 'future_ask_close',

    # Price structure — bid/ask for stochastic, spread features
    ...
    'future_buy_vwap', 'future_sell_vwap',
    'future_price_diff',

    # Book touch level (liq_0.0)
    'future_bid_liq_0.0_median', 'future_ask_liq_0.0_median',

    # Spot flow (for flow_agreement)
    'spot_buy_qty', 'spot_sell_qty',

    # Optional market data
    'opt_open_interest_sample',
    'opt_funding_rate_sample',
    'opt_est_funding_rate_sample',
    'opt_remaining_time_sample',
    'opt_long_force_exit_qty_sum',
    'opt_short_force_exit_qty_sum',

    # Sub-bar trade sequencing (new collector only)
...
# Liq depth columns needed (one level for book_imbal_deep, several for liq_conc)
# Resolved dynamically since not all collectors write all levels.
...
def load_files(files: list[str]) -> pd.DataFrame:
    """
    Load and concatenate CSV files, reading only the columns needed for
    feature computation. Drops ~370 unused columns at read time, reducing
    peak memory from ~5GB to ~300MB for a 1.6M-row dataset.
    """
    all_needed = (COLS_NEEDED_BASE
                  | set(COLS_NEEDED_LIQ_DEEP)
                  | set(COLS_NEEDED_LIQ_TOTAL))
    frames = []
    for path in files:
        try:
            # Peek at header to find which needed columns actually exist
            header_df = pd.read_csv(path, sep=SEPARATOR, nrows=0)
            available = set(header_df.columns)
            usecols   = sorted(all_needed & available)
            if 'spot_datetime' not in usecols:
                print(f"  SKIP {os.path.basename(path)}: missing spot_datetime")
                continue
            df = pd.read_csv(path, sep=SEPARATOR, usecols=usecols,
                             low_memory=False)
            # Some files repeat header as a data row
...
def prepare(df: pd.DataFrame, threshold: float, horizon: int = 1) -> pd.DataFrame:
    ...
    # ── Sub-bar trade sequencing ──────────────────────────────────────────

    # These features require the updated collector (2026-06-21+).
    # Files without these columns produce NaN — model silently drops them.
    # All features use only data from within bar N — no future rows touched.
    ...
    has_sub_bar = all(c in df.columns for c in sub_bar_cols)

    if has_sub_bar:
        fts  = df['future_first_trade_side']
        lts  = df['future_last_trade_side']
        lq   = df['future_largest_trade_qty']
        ls   = df['future_largest_trade_side']
        bce  = df['future_buy_count_early']
        bcl  = df['future_buy_count_late']
        sce  = df['future_sell_count_early']
        scl  = df['future_sell_count_late']
        bqe  = df['future_buy_qty_early']
        bql  = df['future_buy_qty_late']
        sqe  = df['future_sell_qty_early']
        sql  = df['future_sell_qty_late']

        total_vol = bqe + bql + sqe + sql

        # Which side opened and closed the bar
        df['trade_side_open']     = fts
        df['trade_side_close']    = lts

        # Direction change within bar: +2 = opened sell, closed buy (reversal signal)
        ...
        # Volume acceleration: did buying/selling intensify toward bar close?
        df['buy_accel']           = bql - bqe
        df['sell_accel']          = sql - sqe

        # Net flow acceleration: positive = late bar more bullish than early bar
        df['flow_accel']          = (bql - sql) - (bqe - sqe)

        # Buy trade count acceleration (by count, not volume)
        df['buy_count_accel']     = bcl - bce

        # Flow imbalance in the late half only — most recent information in bar
        late_buy  = bql
        late_sell = sql
        df['late_imbalance']      = (late_buy - late_sell) / (late_buy + late_sell + eps)
    else:
        for col in ['trade_side_open', 'trade_side_close', 'trade_side_momentum',
                    'largest_trade_side', 'largest_trade_rel', 'buy_accel',
                    'sell_accel', 'flow_accel', 'buy_count_accel', 'late_imbalance']:
            df[col] = np.nan
    ...
    # ── Drop raw input columns — keep only features + target + metadata ──
    # After feature computation the ~34 raw columns are no longer needed.
    # Dropping them here frees ~200MB on large datasets before the model runs.
    feature_cols = list(FEATURE_DEFS.keys())
    keep = set(feature_cols) | {'dt', 'close_price', 'target', 'delta_usd',
                                 'volatility', 'vol_regime'}
    drop_raw = [c for c in df.columns if c not in keep]
...
def backtest_strategy(df: pd.DataFrame, features: list, horizon: int,
    ...
    try:
        ...
        n_t = int(s['n_trades'])
        if n_t >= 1:
            n_wins  = int(round(n_t * s['wr']))
            binom_p = binomtest(n_wins, n_t, 0.5, alternative='greater').pvalue
            sig     = f"p={binom_p:.4f} {'✓ significant' if binom_p < 0.05 else '✗ not significant'}"
            verdict += f"\n  Win rate {s['wr']*100:.2f}% — binomial test: {sig}"
        else:
            verdict += "\n  No trades to test (all signals filtered by confidence threshold)"
    except ImportError:
        pass
...
def compare_feature_sets(df: pd.DataFrame, features: list):
    """
    Compare XGBoost with original 25 features vs full feature set (including
    sub-bar features), using identical train/test split and hyperparameters.

    Reports:
      - Accuracy and lift for each feature set
      - Rolling window lift stability
      - Which sub-bar features add the most value (by importance delta)
      - Whether the improvement is statistically meaningful
    """
    if not XGBOOST_OK:
        print("  XGBoost not installed — skipping feature set comparison.")
        return

    print_header("FEATURE SET COMPARISON (original 25 vs +sub-bar)")

    # Determine which feature sets are actually available in this dataset
    orig_avail    = [f for f in ORIGINAL_FEATURES if f in features]
    sub_bar_avail = [f for f in SUB_BAR_FEATURES  if f in features]
    full_avail    = orig_avail + sub_bar_avail

    if not sub_bar_avail:
        print("  No sub-bar features available in this dataset.")
        print("  Run with data collected after 2026-06-21 to enable comparison.")
        return

    n_sub = len(sub_bar_avail)
    pct_sub_bar_rows = df[sub_bar_avail[0]].notna().mean() * 100
    print(f"  Original features  : {len(orig_avail)}")
    print(f"  Sub-bar features   : {n_sub} available ({pct_sub_bar_rows:.0f}% of rows have data)")
    print(f"  Full feature set   : {len(full_avail)}")

    if pct_sub_bar_rows < 5:
        print(f"  Only {pct_sub_bar_rows:.1f}% of rows have sub-bar data — too few to compare reliably.")
        print(f"  Collect more data with the updated script and rerun.")
        return

    # Use only rows where ALL sub-bar features are populated for fair comparison
    # (otherwise the full model has systematic NaN rows that the baseline doesn't)
    sub_mask = df[sub_bar_avail].notna().all(axis=1)
    df_sub = df[sub_mask].reset_index(drop=True)
    df_all_rows = df.copy()

    print(f"  Sub-bar rows (for fair comparison): {sub_mask.sum():,} "
          f"({sub_mask.mean()*100:.1f}% of total)")

    le     = {-1: 0, 0: 1, 1: 2}
    le_inv = {0: -1, 1: 0, 2: 1}

    def run_xgb(data, feat_list, label):
        """Train XGBoost on feat_list, return (accuracy, lift, importances)."""
        clean = data[feat_list + ['target']].dropna().reset_index(drop=True)
        if len(clean) < 200:
            return None, None, None
        split  = int(len(clean) * 0.70)
        X      = clean[feat_list].values
        y      = clean['target'].values
        X_tr, X_te = X[:split], X[split:]
        y_tr, y_te = y[:split], y[split:]
        ...
        dummy_acc = max(np.bincount([le[v] for v in y_te]) / len(y_te))
        ...
        xgb.fit(X_tr_s, y_tr_x, eval_set=[(X_te_s, y_te_x)], verbose=False)
        pred   = np.array([le_inv[v] for v in xgb.predict(X_te_s)])
        acc    = (pred == y_te).mean()
        lift   = acc - dummy_acc
        return acc, lift, dict(zip(feat_list, xgb.feature_importances_))

    # Run both models on the sub-bar rows (fair comparison)
    print(f"  Training models on {sub_mask.sum():,} rows where sub-bar data is present...")
    print(f"  (Both models use identical rows and train/test split)")
    print()

    acc_orig, lift_orig, imp_orig = run_xgb(df_sub, orig_avail,    "Original")
    acc_full, lift_full, imp_full = run_xgb(df_sub, full_avail,    "Full")

    if acc_orig is None or acc_full is None:
        print("  Not enough data for comparison.")
        return

    dummy_acc = max(df_sub['target'].value_counts(normalize=True))

    print(f"  {'Feature set':<30}  {'Accuracy':>10}  {'Lift':>10}  {'vs Baseline':>12}")
    print(f"  {'-'*30}  {'-'*10}  {'-'*10}  {'-'*12}")
    print(f"  {'Dummy (majority class)':<30}  {dummy_acc*100:>9.2f}%  {'':>10}  {'':>12}")
    print(f"  {'Original 25 features':<30}  {acc_orig*100:>9.2f}%  {lift_orig*100:>+9.2f}%  {'baseline':>12}")

    delta = lift_full - lift_orig
    arrow = '↑' if delta > 0.001 else ('↓' if delta < -0.001 else '→')
    verdict_acc = f"{delta*100:+.2f}% {arrow}"
    print(f"  {'Full set (+ sub-bar)':<30}  {acc_full*100:>9.2f}%  {lift_full*100:>+9.2f}%  {verdict_acc:>12}")

    print()
    if delta > 0.005:
        print(f"  ✓ Sub-bar features ADD VALUE: +{delta*100:.2f}% lift improvement")
    elif delta > 0.001:
        print(f"  ~ Sub-bar features give marginal improvement: +{delta*100:.2f}%")
    elif delta > -0.001:
        print(f"  → Sub-bar features neutral: {delta*100:+.2f}% (within noise)")
    else:
        print(f"  ✗ Sub-bar features HURT performance: {delta*100:.2f}%")
        print(f"    Possible overfitting — only {pct_sub_bar_rows:.0f}% of data has sub-bar features.")

    # ── Which sub-bar features contribute most ────────────────────────────
    if imp_full and sub_bar_avail:
        print(f"  Sub-bar feature importances (in full model):")
        orig_total    = sum(imp_full.get(f, 0) for f in orig_avail)
        sub_bar_total = sum(imp_full.get(f, 0) for f in sub_bar_avail)
        print(f"  Original features  share: {orig_total*100:.1f}%")
        print(f"  Sub-bar features   share: {sub_bar_total*100:.1f}%")
        print()
        sub_sorted = sorted([(f, imp_full.get(f, 0)) for f in sub_bar_avail],
                            key=lambda x: x[1], reverse=True)
        for feat, imp in sub_sorted:
            bar = '█' * int(imp * 200)
            print(f"    {feat:<25}  {imp:.4f}  {bar}")

    # ── Rolling comparison ────────────────────────────────────────────────
    print(f"  Rolling window comparison ({min(30, len(df_sub)//500)} windows):")
    n_windows   = min(30, max(5, len(df_sub) // 500))
    window_rows = max(200, len(df_sub) // (n_windows + 2))

    orig_lifts, full_lifts = [], []
    for start in range(0, len(df_sub) - window_rows * 2, window_rows):
        w_df = df_sub.iloc[start:start + window_rows * 2].reset_index(drop=True)
        _, l_o, _ = run_xgb(w_df, orig_avail, "orig")
        _, l_f, _ = run_xgb(w_df, full_avail, "full")
        if l_o is not None and l_f is not None:
            orig_lifts.append(l_o)
            full_lifts.append(l_f)

    if orig_lifts:
        orig_mean = np.mean(orig_lifts)
        full_mean = np.mean(full_lifts)
        full_wins = sum(f > o for f, o in zip(full_lifts, orig_lifts))
        print(f"  Original mean lift : {orig_mean*100:+.2f}%")
        print(f"  Full set mean lift : {full_mean*100:+.2f}%")
        print(f"  Full beats original: {full_wins}/{len(orig_lifts)} windows "
              f"({full_wins/len(orig_lifts)*100:.0f}%)")

        if full_wins / len(orig_lifts) > 0.6:
            print(f"  ✓ Sub-bar features consistently improve predictions")
        elif full_wins / len(orig_lifts) > 0.4:
            print(f"  ~ Sub-bar features help in some windows but not consistently")
        else:
            print(f"  ✗ Original features outperform in most windows")
...
def main():
    ...
    parser.add_argument('--compare',    action='store_true',     help='Compare original 25 features vs full set with sub-bar features')
